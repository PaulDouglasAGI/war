...
MID_LABELS = (
    ...
    'NEUTRAL'.center(MID_END - LEFT_END) + '│' +
...
def render(frame):
    ...
    for y in range(HEIGHT):
        for x in range(WIDTH):
            cell_strings[y][x] = terrain[y][x]
    # Place HQs
    for team, (hx, hy) in HQ_POS.items():
        cell_strings[hy][hx] = 'B'
    # Place units
    for u in units:
    ...
    print(f"{Fore.CYAN}TEAM A{Style.RESET_ALL}: {blue_cnt} units | {resources['blue']} res")
    print(f"{Fore.RED}TEAM B{Style.RESET_ALL}: {red_cnt} units | {resources['red']} res")
    print(f"Frame: {frame}")
